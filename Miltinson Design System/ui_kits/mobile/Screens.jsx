...
const PhoneFrame = ({ children, label }) => (
  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 14 }}>
    <div style={{
      ...
      padding: 10, boxShadow: '0 24px 60px -16px oklch(0% 0 0 / 0.3)'
    }}>
      <div style={{
        ...
        <div style={{
          ...
          <span style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
            <span style={{ width: 18, height: 11, border: '1.5px solid currentColor', borderRadius: 2, position: 'relative' }}>
    ...
    <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 11, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--fg-3)' }}>{label}</div>
  </div>
);
  ...
  return (
    <div style={{ padding: '8px 20px 20px', overflow: 'auto', height: 'calc(100% - 36px)' }}>
      <div style={{
        ...
        color: 'var(--signal-700)', marginBottom: 6
      }}>Kids Recipes</div>
      <h1 style={{
        fontFamily: 'Geist, sans-serif', fontWeight: 600,
        fontSize: 30, letterSpacing: '-0.02em', margin: '0 0 6px',
        lineHeight: 1.1
      }}>What's for breakfast?</h1>
      <p style={{ fontFamily: 'Geist, sans-serif', fontSize: 13, color: 'var(--fg-2)', margin: '0 0 18px' }}>Pick a recipe, get cooking.</p>
        ...
        {recipes.map((r, i) => (
          <div key={i} style={{
            ...
            <div style={{
              ...
              background: 'var(--signal-50)', display: 'grid', placeItems: 'center',
              fontSize: 28
            }}>{r.emoji}</div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontFamily: 'Geist, sans-serif', fontSize: 14, fontWeight: 600, letterSpacing: '-0.01em' }}>{r.name}</div>
              <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 10, color: 'var(--fg-3)', letterSpacing: '0.06em', textTransform: 'uppercase', marginTop: 4 }}>{r.tag} · {r.time} · {r.diff}</div>
            </div>
          </div>
        ))}
      </div>
...
const MathsScreen = () => {
  return (
    <div style={{ padding: '8px 20px 20px', height: 'calc(100% - 36px)', display: 'flex', flexDirection: 'column' }}>
      <div style={{
        display: 'flex', justifyContent: 'space-between', alignItems: 'center',
        fontFamily: 'JetBrains Mono, monospace', fontSize: 11, color: 'var(--fg-2)', marginBottom: 22
      }}>
        <span style={{ letterSpacing: '0.16em', textTransform: 'uppercase' }}>Question 4 / 10</span>
        <span style={{ color: 'var(--anchor-500)' }}>Streak ×3</span>
      </div>
      <div style={{
        height: 4, background: 'var(--ink-100)', borderRadius: 999, marginBottom: 36, overflow: 'hidden'
      }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', textAlign: 'center' }}>
        ...
        }}>Multiplication · Year 4</div>
        <div style={{
          fontFamily: 'JetBrains Mono, monospace', fontWeight: 500,
          fontSize: 56, letterSpacing: '-0.02em', color: 'var(--ink-1000)',
          lineHeight: 1
        }}>7 × 8 = ?</div>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 12 }}>
        {['54', '56', '63', '49'].map((v, i) => (
          <button key={v} style={{
            ...
            color: 'var(--ink-1000)',
            fontFamily: 'JetBrains Mono, monospace', fontWeight: 500,
            fontSize: 22, letterSpacing: '-0.01em', cursor: 'pointer'
          }}>{v}</button>
        ))}
      </div>
      <button style={{
