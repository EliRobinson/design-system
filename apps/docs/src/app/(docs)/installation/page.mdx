...
Authenticate once, install two packages, import one stylesheet — that's the whole setup.
...
[Build with AI](/build-with-ai) covers it. `@elirobinson/ai-elements` is the assistant tier:
...
do not — see [AI Elements](/components/ai-elements/overview).

## Wire up the styles

Import the component stylesheet once, in your app shell:

```tsx
import '@elirobinson/react/styles.css';
```

`styles.css` is the aggregate component stylesheet, and its first line imports
`@elirobinson/tokens/tokens.css`: every custom property on `:root` plus sensible base styles
(focus rings, selection color, reduced-motion handling). Do not import `tokens.css` as well.
Bundlers keep both copies, and the later one wins every tie against your own CSS between
them. `@elirobinson/eslint-config` flags the two imports in one file.

Not importing `styles.css` (the tokens without React, or only per-component sheets)? Import
`@elirobinson/tokens/tokens.css` once yourself: the per-component sheets do not bring it.

On Tailwind v4, put the import in your CSS entry instead of a layout module, between
`@import 'tailwindcss'` and `@import '@elirobinson/tokens/tailwind.css'`. If you'd rather ship only what you use, each component's sheet is importable on
its own:
