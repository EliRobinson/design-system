...
Using `@elirobinson/react` too? Put `@import '@elirobinson/react/styles.css';` on the second
line instead of `tokens.css`. It opens with `tokens.css`, so importing both bundles the
tokens twice.
...
- `tokens.css` declares the custom properties. It has to be loaded — the next file aliases
  its variables, it does not define them. With `@elirobinson/react`, `styles.css` loads it.
...
Elements carries no stylesheet of its own. If you also use `@elirobinson/react`, its
stylesheet takes the place of the `tokens.css` line above rather than going in beside it:
`styles.css` opens with `@import '@elirobinson/tokens/tokens.css'`, and a second import is a
second copy of the tokens.
...
@import '@elirobinson/tokens/tokens.css'; /* or @elirobinson/react/styles.css */
@import '@elirobinson/tokens/tailwind.css';
```
