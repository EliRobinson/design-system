...
- `PromptInput` renders a real `<form>` — but `onSubmit` is required and its handler calls
  `preventDefault()` unconditionally, so nothing here degrades without JavaScript. What the
  form buys you is one path: `requestSubmit()` runs native constraint validation, and `Enter`
  and the send button both go through it.
