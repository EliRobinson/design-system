...
- **`Confirmation` renders the shared `Alert`, which sets `role="alert"`** — an assertive
  live region. So the gate announces itself on insertion whatever `aria-live` you put on the
  thread around it. It is the only component in this family that lands inside a live region,
  and it is the one where that is probably right.
