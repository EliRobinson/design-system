...
- A menu that doesn't fit below its trigger flips above it, and one that overruns the right
  ...
  the preferred side.
- A menu that fits on neither side is shifted rather than shrunk: it keeps its width and
  ...
  which is the price. The width it keeps has a floor — `.ds-dropdown__content` sets
  `--anchored-min-width: 180px`, and the positioner keeps the larger of that and the trigger's
  width. A menu taller than the viewport is the one case no offset rescues: it takes a
  `max-height` of the viewport and scrolls its own items.
