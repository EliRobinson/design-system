export const metadata = { title: 'Sidebar pattern', navTitle: 'Sidebar', order: 4 };
...
- Section labels (if you add them above groups) are mono uppercase eyebrows, not
  headings. Add them as items with no `href` — a header carrying its first child's href
  renders as the current page whenever that child is open.
