 ...
 * colour value.
 *
 * Motion. `Conversation` animates its own scroll on mount and on resize, and
 * that finding lives in `scripts/ai-elements-patches/motion.mjs` instead: it
 * is not a control measured against a touch-target floor, so it has no
 * `primary`/`dense` verdict to publish here, and `contracts.json` /
 * `elements-classification-parity.test.mjs` both hold this file's entries to
 * exactly those two tiers and no others.
 */
