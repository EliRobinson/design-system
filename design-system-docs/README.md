...
| Path                        | Ships | What's there |
| --------------------------- | ----- | --- |
| `colors_and_type.css`       | ✓ | The neutral ramp, surfaces, type, spacing, radii, shadow and motion — and the entry point that @imports the two layers below. Symlink to `packages/tokens/src/tokens.css`. |
| `palettes.css`              | ✓ | The palette layer — brand ramps and every semantic derived from them, per `data-palette` and `data-theme`. colors_and_type.css @imports it, so it must keep this filename. Symlink to `packages/tokens/src/palettes.css`. |
| `mobile.css`                | ✓ | The platform layer — radii, the small end of the type ramp, gutter and containers under `data-platform="mobile"`. Declares no token of its own and changes no colour. Symlink to `packages/tokens/src/mobile.css`. |
| `styles.css`                | — | Generated aggregate the guideline cards link — @imports the real component stylesheets from packages/react. Rebuilt by `pnpm -F @elirobinson/ai-patterns build:design-project`. |
| `fonts.css`                 | ✓ | @font-face for Geist and JetBrains Mono, self-hosted — colors_and_type.css @imports it. Symlink to `packages/tokens/src/fonts.css`. |
| `fonts/`                    | ✓ | The woff2 files fonts.css loads, with their SIL OFL licenses. Symlinks into `packages/tokens/src/fonts/`. |
| `assets/`                   | ✓ | Wordmark, monogram, lockup, dot-grid pattern. SVG-first. |
| `guidelines/`               | — | 13 token-enumeration cards generated by guideline-cards.mjs, the brand voice card generated by sync-voice.mjs, plus 9 hand-authored editorial cards. |
| `preview/`                  | — | Design System tab cards — type/color/spacing/component swatches. Working material. |
| `components/`               | — | 2 component specimen cards — one per tier group, showing the real `ds-*` markup against the aggregate stylesheet. Working material. |
| `ui_kits/marketing/`        | ✓ | Marketing UI Kit — Header, Footer, Hero, FeaturedApps, FeatureBand, ServicesBand. |
| `ui_kits/webapp/`           | ✓ | Web App UI Kit — Sidebar, TopBar, StatCard, ProjectsTable. |
| `ui_kits/mobile/`           | ✓ | Mobile UI Kit — PhoneFrame, BrowseScreen, PracticeScreen. |
| `ui_kits/docs/`             | ✓ | Docs / Guide UI Kit. |
| `ui_kits/_shared/`          | ✓ | Shared JSX primitives every kit loads, and content.js — every string the kits render, in one file. |
| `slides/`                   | — | 16:9 deck templates — CloserSlide, ContentSlide, QuoteSlide, TitleSlide — plus an index. |
| `patterns/email/`           | — | Email template. |
| `patterns/invoice/`         | — | Invoice & receipt. |
| `patterns/social-carousel/` | — | Social carousel template. |
