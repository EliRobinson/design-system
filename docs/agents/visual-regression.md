...
Condition **(B)** was #100: the sweep is a matrix of per-project jobs and `scripts/visual-matrix.mjs` carries `docs-wide: 2` in `SHARDS`, so the project runs as two concurrent shards, `sweep (docs-wide 1/2)` and `sweep (docs-wide 2/2)`, alongside the storybook jobs rather than ahead of them. Two, not three: 160 tests at ~1.37s is ~219s serial and ~110s a shard, level with `storybook-wide`'s 107s. Clipping does not make a shot cheaper — the settle loop still captures the whole page, deliberately, and that is the expensive half — so a third shard would buy about three seconds of wall clock for a machine-minute.
