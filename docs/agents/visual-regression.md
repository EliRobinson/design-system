...
| trigger                                       | workflow        | job(s)                                                                         | what runs                                                                                               |
| --------------------------------------------- | --------------- | ------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------- |
| pull request opened, synced, or reopened      | `visual-pr.yml` | `plan` → `compare (<project>)` ×N → `scoped`                                   | only the shots the change could have altered, plus any shot that has no baseline yet                    |
| `visual-accept` label added to a pull request | `visual-pr.yml` | the same three — a fresh run, since adding the label is itself a trigger       | the same scoped comparison, then `scoped` regenerates and commits exactly the shots that failed         |
| push to `main`                                | `visual.yml`    | `build` → `sweep (<project>)` ×N → `full`, then `recover` if it goes red       | the full sweep, split by project across concurrent jobs                                                 |
| nightly, 06:00 UTC                            | `visual.yml`    | the same four                                                                  | the full sweep                                                                                          |
| `workflow_dispatch`                           | `visual.yml`    | the same four (`recover` only fires if the ref is `main` and the sweep failed) | the full sweep — this dispatch takes no `--grep` input; use `visual-update.yml` for a scoped manual run |

**Pull requests and `main` run two separate workflows.** They used to be two paths through `visual.yml`, and every pull request listed `build`, `sweep`, `full` and `recover` as skipped checks that could never run for that event. A job's `if:` is for a decision made at run time; which event a job belongs to is not one. Two names are load-bearing across the split: Release triggers on the workflow named `Visual`, so the pull request workflow is `Visual PR`; and the `main` ruleset requires the contexts `image` and `scoped`, so both job ids survive in `visual-pr.yml`.

**The pull request comparison is a matrix too.** `plan` builds both apps, runs the scoper, mints missing baselines, and then plans the matrix with `node scripts/visual-matrix.mjs --grep-file scope-grep.txt` — Playwright's own enumeration of just the scoped shots. A scope of `SCOPED_SINGLE_JOB_MAX` (40) shots or fewer is one `compare` job, because each job pays ~80s of start-up before it takes a pixel; a larger scope gets the same per-project jobs and shards as the full sweep, which is the case that was slow — a token or palette change scopes to every shot and took 10+ minutes serially. Each `compare` leg writes a blob report and decides nothing, and holds no write credential. `scoped` merges the legs, checks every planned project reported, and then passes, accepts, or reports. Pull request jobs restore the pnpm and Nx caches but never save them. `visual.yml`'s `build` saves the Nx cache only on the nightly run: the save is a ~70MB upload that every `sweep` job used to wait on (101s measured on run 35190180037), and a day-old entry costs only a rebuild of what changed since.
...
Any other label added to a pull request also re-triggers `visual-pr.yml` — GitHub's `labeled` event can't be filtered to one label name in the workflow's `on:` block, so the `plan` and `scoped` jobs check `github.event.label.name == 'visual-accept'` and no-ops for anything else. Worth knowing before you debug it twice: **adding any label cancels whatever scoped run is already in flight**, because the workflow-level `concurrency` group is evaluated before the job's `if:` — a `wip` or `needs-triage` label restarts the run and then does nothing with it.

**A new shot is minted automatically, on the pull request branch.** The `plan` job runs `scripts/visual-missing.mjs`, and for anything with no baseline runs Playwright with `--update-snapshots=missing` and commits the result before the comparison even starts. This can only ever _create_ a baseline — Playwright's `missing` mode refuses to touch a shot that already has one — so it cannot launder a regression into an accepted baseline. A component-adding pull request no longer needs a manual `visual-update` dispatch for its new shots.

**Overwriting an existing baseline needs the `visual-accept` label, and the label accepts once.** That separation is the whole point: minting is automatic and can only create, accepting is opt-in and can overwrite. Add the label and the workflow starts a new run on its own — there is no need to re-run the job by hand. On that run, each red `compare` leg reruns only its own failing shots with `--update-snapshots=changed` and uploads the regenerated files, so accepting a palette change regenerates across the whole matrix at once. `scoped` then commits exactly the baselines of the shots the merged report lists as failing (`scripts/visual-failures.mjs --baselines`) as `test(visual): accept N baselines` — and refuses to commit anything if one of those shots came back unregenerated, because its leg crashed, timed out, or rendered it identically on the rerun (#65's flake).
...
**A fork's pull request is refused before it runs anything.** The baseline bot's installation token is scoped to this repository and can't push to a fork's branch, so every mint, accept, and commit step in `visual-pr.yml` would fail partway through. `plan` checks this first and fails fast with the same `visual-update.yml --ref ... --field grep=...` remedy, and `scoped` goes red naming `plan`. Nothing in `visual.yml` needs this check: it never runs on a pull request.

**None of this is where the shot list itself is checked — `scripts:test` is.** `visual-shots.test.mjs` holds every baseline on disk against the list Playwright enumerates, and `visual-scope.test.mjs` applies a scoping pattern with Playwright itself; between them they are what reports Playwright changing how it resolves `{testFilePath}`, or a viewport project added to `playwright.config.ts` and not to `SPEC_FILE_BY_PROJECT`. Both spawn `playwright test --list`, so both read `apps/storybook/storybook-static/index.json` and `apps/docs/.next/prerender-manifest.json` — **which means `scripts:test` is only meaningful after both apps are built.** `scripts/project.json` names storybook and docs in `implicitDependencies` for exactly that, which is what makes nx.json's `test: dependsOn ["^build", "build"]` resolve to their builds. Do not remove it to make the task cheaper: without it Nx scheduled `scripts:test` with nothing to wait for, next to a `storybook:build` that empties `storybook-static` before refilling it, and the suite read a directory that was missing (both describes skip, green having checked nothing), half-written (`--list` exits non-zero), or one commit stale (the parity assertion fails on the difference). The last shape is why the task read as flaky rather than broken — on 2026-08-26 a `storybook-static` predating #177's DatePicker `Open` story computed 570 shots against 574 baselines, naming exactly that story's four. `buildsPresent()` in `scripts/visual-builds.mjs` now throws rather than skipping when a build is missing under an Nx task, so a regression in that ordering reports itself instead of going quiet.

### Adding or removing a component runs every docs shot, and that is correct

`apps/docs/src/lib/site-map.ts` builds one page registry, and the sidebar, the header nav, the homepage cards, and the command palette all derive from that registry — not from the manifest directly. Only the Components sections of the registry come from the generated component manifest; Foundations, Patterns, and Guidelines come from the `page.mdx` files on disk instead (see `derivedSection()` in that file). Either source feeds the same registry, and the sidebar renders on every docs page. So **adding or removing a component can change any docs shot**, and the scoper cannot tell in advance which: measured on PR #88, which added six components (ChatMessage, ChatThread, DecisionCard, StreamingCaret, StubCard, VerdictBadge), all 142 pre-existing docs-wide screenshots failed comparison under the full-page framing of the time, and zero stories did.

This is why `scripts/visual-scope.mjs` treats an added or deleted component file — and an added or deleted docs page file, and any change to `site-map.ts`, `manifest.ts`, the `(docs)` layout, `SiteHeader`, or `site.css` — as a docs-wide event rather than narrowing to that component's own page. Narrowing there would produce a green pull request and stale baselines on `main`. A _modified_ component file doesn't trigger this: the registry is unchanged, so the sidebar is unchanged, and narrowing to that component's own shots stays safe.

**Editing** a page under `foundations/`, `patterns/`, `guidelines/`, or `components/ai-elements/` is a docs-wide event too, unlike editing a component page. Those four sections are the derived ones: `derivedSection()` reads each page's own `metadata.title`, `navTitle`, and `order` off disk to build the sidebar entry, so retitling or reordering one page moves an entry that renders on every docs page. That includes a `page.mdx` at a section's root, which `derivedSection()` treats as a page of the section like any other — only AI Elements has one. A component page carries no such metadata — its sidebar entry comes from the generated manifest — so editing one stays narrowed to that page. AI Elements is the awkward case, being a derived section that lives under `components/`: `DERIVED_SECTION_PAGE` names its prefix literally so the derived rule wins there and nowhere else under `components/`.
...
A global stylesheet counts, whether or not it has churned a baseline yet. `apps/docs/src/app/elements.css` is imported from the **root** layout and starts with `@import 'tailwindcss'`, so it carries Preflight onto all 136 docs pages; it is in `SIDEBAR_SOURCES` for that reason. It changed nothing when it landed, and the reason is worth writing down rather than trusting: Tailwind v4 puts Preflight in `@layer base`, and `site.css` is unlayered, so `site.css` beats it regardless of import order. That is a property of today's CSS, not a guarantee — layer `site.css`, or add an unlayered rule to `elements.css`, and the two swap places.

All of that is about which shots **run**. Which shots **fail** is a separate question, and it is the one that made `docs-wide` untenable between #101 and #105: under full-page framing every one of those 142 runs also failed, so "expected, not a break" cost a long run and a `visual-accept` label on every component pull request. The matrix made the run cheap; the framing in the next section is what made the failure set honest. A component-adding branch now reds the sidebar's one chrome shot and the pages that render registry data, not the whole docs project.

### How `docs-wide` is framed, and why

**A docs page shot is the page's content region, not the whole page.** `tests/visual/docs/docs.spec.ts` passes `region: 'main'` to `sweepPages`, so every page shot is clipped to the content element and the header, nav, sidebar and footer are outside the frame. Those get their own shots — three regions, two themes, on one route — from `sweepChrome`, in the `/chrome/*` shot namespace.

This is the answer to condition (A) of #101, decided in #105. `docs-wide` had been off since #101 because the sidebar derives from one registry and renders on every docs page, so one added component invalidated all 142 shots at once (measured on #88: 142 docs failures, 0 story failures) — a project reliably red on exactly the pull requests this system exists to protect, reporting one bit repeated 142 times.

Three options were on the table, and the reasoning for picking this one is worth keeping:

> Masking and registry-stubbing both keep the sidebar inside the frame and try to make the comparison ignore it. Masking still lets a sidebar width change shift the content behind the mask, so it suppresses the fan-out rather than removing it. Stubbing the registry removes it but renders a component list production never has — a new way for the suite to disagree with production, and nobody is watching that axis. Clipping is the only option where the fan-out is structurally impossible: the sidebar is outside the frame, so it cannot fail a page shot. The listed downside — needing dedicated chrome shots — is a benefit. One sidebar shot is the correct amount of sidebar coverage; 142 was a side effect of full-page framing, never a design decision.

Measured on the same one-component perturbation, over a 15-subject sample in both themes:

| framing                   | shots changed |
| ------------------------- | ------------- |
| full page (before #105)   | 30 of 30      |
| clipped to `main` (after) | 8 of 30       |

The eight are `/`, `/installation`, `/components` and `/chrome/sidebar`, in both themes — the home page and the installation page render a component count, the index page lists every component, and the sidebar gained an entry. Every one of them genuinely moved. The other twenty-two are byte-identical.

**Real fan-out is untouched.** The property `visual-scope.mjs` protects is that a change which genuinely alters every page still fails every page, and clipping does not weaken it: a global `--fg` / `--fg-2` change over the same sample failed 28 of 30, which is every page shot plus the header and sidebar. The two that held were the footer's, correctly — it paints from `--fg-inverse`.

Four things worth knowing about the framing:

- **Adding a component still costs one `visual-accept`**, for `/chrome/sidebar` and for the pages that render registry data. That is the sidebar genuinely changing, in the one baseline that shows it, rather than in 142.
- **`visual-scope.mjs` still fans out to every docs route** for a registry change, and should. Which shots _run_ is a cost question the matrix now absorbs; which shots _fail_ is the signal, and that is what clipping fixed. Narrowing the scoper instead would produce a green pull request and stale baselines.
- **The sidebar shot is the sidebar a visitor sees.** `site.css` clamps it to `calc(100vh - …)` with `overflow-y: auto`, so its box is ~696px of a ~4164px list. That is not a reduction: a full-page capture clipped it to the same box, so this shot compares the same sidebar pixels the deleted baselines did. The rest of the list is compared where it is genuinely rendered in full, on `/components`.
- **What clipping gives up** is the page's outer geometry. The clip follows the element, so moving the content region as a whole — a change to `.docs-shell`'s padding, say — produces an identical shot. The chrome shots cover the header and footer boxes; the gap between them and the content is not compared by anything. That is the honest cost of the trade.

Condition **(B)** was #100: the sweep is a matrix of per-project jobs, and `docs-wide` runs as concurrent shards alongside the storybook jobs rather than ahead of them. It started at two. By 2026-09-17 the two shards took 213s and 123s — Playwright balances shards by test count, not duration — so `SHARDS` in `scripts/visual-matrix.mjs` now gives `docs-wide` four and each storybook project two, sized so no leg's screenshots take much more than ~90s. The machine-minute argument that once held the count at two no longer applies: the repository is public, so standard runners are not billed. The limit left is the account's 20 concurrent jobs, shared with every other workflow. Clipping does not make a shot cheaper — the settle loop still captures the whole page, deliberately, and that is the expensive half.

## The baseline bot, and why the pushes need it

Every workflow here that commits baselines — `visual-pr.yml`'s mint (in `plan`) and accept (in `scoped`) steps, `recover`'s
...
**`plan` fails loudly if either secret is missing**, and `scoped` goes red with it,, rather than falling back to
...
Cost is one extra `visual-pr.yml` run per mint or accept.

---

**Switching a project on or off is one block in `playwright.config.ts`, and nothing else.** That is enforced rather than hoped for: the matrix is derived from the projects Playwright actually collected, so re-enabling `docs-wide` in #105 gave it two sharded jobs with no edit to `visual.yml` or to `scripts/visual-matrix.mjs` — `scripts/visual-matrix.test.mjs` covers both the absent case and the sharded one. Every project that takes baselines stays in `SPEC_FILE_BY_PROJECT` whether it is running or not, so the path mapping survives the round trip.

**Baselines for a project that comes back are minted on the runner.** The mint step in `visual-pr.yml` writes any shot with no baseline and commits it to the pull request branch, on the first run. Do not regenerate them locally — see the top of this file for why the host is the variable, and `assertContainedBaselineUpdate` in `packages/ai-patterns/src/testing/visual-config.mjs` for the guard that refuses to let you.

**A framing change deletes the baselines it invalidates rather than accepting them.** #101 deleted `docs-wide`'s 142 when the project went off; #105 deleted `docs-narrow`'s 14 when the clip changed what those shots frame. Comparing a content-region shot against a full-page baseline fails on size for every single one, so an accept round-trip would show a reviewer nothing it did not already know — and until the mint lands, `visual-shots.test.mjs`'s parity check reads red on the pull request, which is the same self-healing state a component-adding branch has always produced.
...
- **The visual workflows report honestly.** Neither `scoped` nor `full` takes screenshots any more: each needs its matrix of comparison jobs and reads the matrix's result, so a later step can still run after a red comparison: `scoped`'s accept-and-commit step, or `full`'s regenerate-and-upload step. On a red `compare`, `scoped` runs exactly one of two mutually exclusive steps: "Accept the changed baselines" if `github.event.action == 'labeled'` — THIS run is the one where `visual-accept` was just applied, not merely a run where the label is sitting on the pull request — or "Report the failing shots", which ends the job with `exit 1`, on every other action. The Accept step is not a guaranteed route to green: it exits non-zero itself if `scripts/visual-failures.mjs` lists no failing shot (the check failed for a reason with no baseline to accept — a crash or a build break) or if regenerating the failing shots produces no change on disk (the check failed for a reason accepting can't fix). So a `labeled` run can still end red — a red `scoped` job is not proof the label failed to apply. `scoped` has an explicit "Decide the verdict" step and `full` a "Fail the job" step for the same red-job guarantee. A red job is a red job.
