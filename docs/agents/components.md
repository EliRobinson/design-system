...
- Touch targets are scoped by control role, not one blanket size:
  ...
  - **On a phone the primary tier rises to 44px; the dense tier does not move.** `tokens.css` floors controls to `--target` under `data-platform="mobile"` and, identically, under `@media (max-width: 480px) and (pointer: coarse)`. Both halves exclude the dense affordances by name, so a chip stays 32px and a `size="sm"` button stays 36px on a phone as on a desktop — they are held to 24x24 there as everywhere, which is a measurement, not an exemption. Inflating them on mobile would discard the scale they were drawn at (MUI's Chip, shadcn's `sm`) and buy nothing the dense floor was not already buying. The exclusion list is the same list as `DENSE_AFFORDANCE_SELECTOR`, and `dense-selector-parity.test.mjs` fails if the two drift apart.
  - **A decorative glyph inside a control is drawn, not typed.** A `<button>`, `<input>`, `<select>` and `<textarea>` do not inherit `font-family` — the UA stylesheet sets it — so a control that renders a literal `×`, `★` or `‹` paints it in whatever font the UA chose. Declaring the family fixes the typeface but not the position: `align-items: center` centres a text node's line _box_, and where the ink sits inside it is a property of that family's metrics, so the same rule centres two families by two different amounts and moves again if a consumer re-points `--font-sans`. Use `Mark` from `lib/marks.tsx` instead — inline SVG, `currentColor`, `aria-hidden`, sized from a `--mark-size` the control sets, geometry symmetric about its own centre — and the offset is zero by construction. `.ds-chip__remove` is the worked example; the other glyph-bearing controls adopt it next. Controls whose content is genuinely _text_ want a declared `font-family`, not a mark.
...
- **`FormField`** (`molecules/FormField`) wraps an arbitrary or third-party control that
  ...
  duplicate labels.
- `Input`, `Textarea`, `Select` and `FormField` all render their error with
  `role="alert"`, in a slot of its own so a hint turning into an error mounts a fresh
  live region. Keep the four agreeing: a consumer should never have to render the
  error themselves to get it announced.
...
| `DatePicker`                    | organisms | Popover date grid (`role="grid"`/`row`/`columnheader`/`gridcell`, weekday header row, dense day-cell sizing); `label` required, `value`/`onValueChange`.                                                                                                                                                                                                                                  |
...
| Calendar / Date Picker                                   | `DatePicker` — popover date grid, ARIA `grid`/`row`/`columnheader`/`gridcell`                                                                                       |
