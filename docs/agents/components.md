...
| `CommandPalette`                | organisms | `Dialog`/`DialogContent`/`DialogTitle`-backed command list with a filterable `SearchField` and `Kbd` shortcut hints; `isOpen`/`onOpenChange`/`commands`.                                                                                                       |
