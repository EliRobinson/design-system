...
- **Data — no.** `--chart-1` … `--chart-8` are eight hues at one lightness and one chroma, so no series shouts louder than another, ordered so consecutive slots are at least 90° apart. `chart-palette.test.mjs` holds both themes to the dataviz palette validator: lightness inside the theme's band, and ΔE 8 or more under simulated protanopia and deuteranopia for every adjacent pair and among the first three slots. A chart does not restyle itself when the brand does. Plus `--chart-grid` (decorative, the `--border-strong` of a plot) and `--chart-axis` (3:1, a line that carries meaning).
