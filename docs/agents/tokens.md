...
| -------- | ----------------------------- | ----------------------------------------------------------------------------------------------------------- | ----------------------------- |
| Palette  | `data-palette="ember\|slate"` | the neutral hue and chroma, `--signal-*`, `--anchor-*`, `--accent*`, `--anchor*`, `--link*`, `--focus-ring` | `palettes.css`                |
...
`contrast.test.mjs` asserts the independence directly: every `--status-*` and `--chart-*` must resolve to the same value under both palettes, and `--accent` must not — so the assertion cannot pass on a stylesheet where nothing varies at all.
...
- Every combination is measured, not every theme. `contrast.test.mjs` resolves `var()` chains per palette × theme — four columns, not two — because the un-inverted tints passed a two-theme sweep the whole time they were broken.
