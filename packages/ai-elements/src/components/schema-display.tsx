 ...
 * Source:    packages/elements/src/schema-display.tsx
...
const methodStyles: Record<HttpMethod, string> = {
  DELETE: "bg-destructive-tint text-destructive-ink",
  GET: "bg-success-tint text-success-ink",
  PATCH:
    "bg-warning-tint text-warning-ink",
  POST: "bg-info-tint text-info-ink",
  PUT: "bg-accent-tint text-accent-ink",
};
...
export const SchemaDisplayPath = ({
  ...
  const highlightedPath = path.replaceAll(
    /\{([^}]+)\}/g,
    '<span class="text-info-ink">{$1}</span>'
  );
...
export const SchemaDisplayParameter = ({
  ...
  <div className={cn("px-4 py-3 pl-10", className)} {...props}>
    <div className="flex items-center gap-2">
      ...
      {required && (
        <Badge
          className="bg-destructive-tint text-destructive-ink text-xs"
          variant="secondary"
        >
          required
        </Badge>
