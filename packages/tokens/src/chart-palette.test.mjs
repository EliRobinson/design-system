...
function linearRgb(value) {
  const { r, g, b } = parseColor(toHex(value));
  return [r, g, b].map(linear);
}
...
function deltaE(first, second, kind) {
  const a = oklab(simulate(linearRgb(first), kind));
  const b = oklab(simulate(linearRgb(second), kind));
  return 100 * Math.hypot(a[0] - b[0], a[1] - b[1], a[2] - b[2]);
}

const sources = readTokenStylesheets();

/* Adjacent slots, then every pair among the first three that is not already
   adjacent — which is only 1↔3, the pair #252 was filed for. */
const PAIRS = [...Array.from({ length: SLOTS - 1 }, (_, i) => [i + 1, i + 2]), [1, 3]];

describe('the chart ramp passes the dataviz palette validator', () => {
  for (const combination of COMBINATIONS) {
    const values = combinationValues(sources, combination);
    const series = Array.from({ length: SLOTS }, (_, i) => values.get(`--chart-${i + 1}`));
    const { id, theme } = combination;

    it(`${id}: every --chart-* is declared and measurable`, () => {
      for (const value of series) expect(toHex(value ?? '')).not.toBeNull();
    });

    it(`${id}: every --chart-* sits inside the ${theme} lightness band and above the chroma floor`, () => {
      const [lo, hi] = BAND[theme];
      series.forEach((value, i) => {
        const [L, a, b] = oklab(linearRgb(value));
