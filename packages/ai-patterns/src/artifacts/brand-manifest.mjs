...
/* The one guideline card buildGuidelineCards does not emit: sync-voice.mjs
   writes it from the voice pack (see voice/render.mjs's renderVoiceCard), so
   it is generated too, just by a different generator. */
const VOICE_CARD_PATH = 'guidelines/brand-voice.html';
const VOICE_SYNC_SCRIPT = 'packages/ai-patterns/scripts/sync-voice.mjs';
...
export function buildBrandManifest({ root, tokens }) {
  ...
  /* -- Guideline cards. Origin comes from buildGuidelineCards — the paths it
        emits are generated, VOICE_CARD_PATH is generated by sync-voice.mjs
        instead, and everything else in the folder is editorial writing. File
        inspection cannot recover this distinction. */
  for (const file of under('guidelines')) {
    ...
    const voiceGenerated = file.path === VOICE_CARD_PATH;
    add({
      ...
      group: card.group,
      origin: generated || voiceGenerated ? 'generated' : 'hand-authored',
      generatedBy: generated ? GUIDELINE_CARDS_MODULE : voiceGenerated ? VOICE_SYNC_SCRIPT : null,
      members: [{ path: file.path, role: 'entry' }],
...
export function renderRepoIndexTable(manifest) {
  ...
  const tokenEnumerationCards = guidelineCards.filter(
    (card) => card.generatedBy === GUIDELINE_CARDS_MODULE,
  ).length;
  const handAuthoredCards = guidelineCards.length - generatedCards;
  ...
  const rows = [
    ...
    },
    {
      path: 'guidelines/',
      description:
        `${tokenEnumerationCards} token-enumeration cards generated by guideline-cards.mjs, ` +
        `the brand voice card generated by sync-voice.mjs, plus ${handAuthoredCards} ` +
        'hand-authored editorial cards.',
    },
    {
      path: 'preview/',
      description:
